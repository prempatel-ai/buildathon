...
class CampaignOfferItemResponse(BaseModel):
    ...
    discount_type: str = Field("percentage", description="'percentage' or 'fixed'")
...
class MerchantCampaignPerformanceResponse(BaseModel):
    merchant_id: str
    merchant_name: str
    offers_generated: int = Field(0, description="Total abandonment re-engagement offers generated")
    offers_shown: int = Field(0, description="Offers delivered to shoppers in chat")
    offers_converted: int = Field(0, description="Offers successfully converted to settled purchases")
    conversion_rate: float = Field(0.0, description="Percentage of shown offers converted (e.g. 25.0%)")
    total_discount_given: float = Field(0.0, description="Total discount value subsidized in INR")
    total_attributed_revenue: float = Field(0.0, description="Total settled GMV attributed to campaign offers in INR")
